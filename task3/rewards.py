...
def dormant_to_active(state, prob, index):
    ...
    # return prob*0.2*(step_cost + kill_monster_reward(state[3])) + prob*0.8*( step_cost + kill_monster_reward(state[3]))
    return step_cost*prob
    # return [ [ [ state[0],state[1],state[2],state[3],1 ], prob*0.2 ]  , [  [state[0],state[1],state[2],state[3],0] , prob*0.8  ] ]
    # return [ [  state[0]+state[1]+state[2]+state[3]+1 , prob*0.2 ]  , [  state[0]+state[1]+state[2]+state[3]+0 , prob*0.8  ] ]
...
def active_to_dormant(init_pos, init_state ,state, prob, index):
    if(init_pos==4 or init_pos==2):
        ...
        return 0.5*prob*( -40 + step_cost ) + 0.5*prob*( step_cost + kill_monster_reward(state[3]))
        # return [  [ [init_pos, 0, init_state[2], min(4,init_state[3] + 1), 0] , 0.5*prob ] , [ [state[0],state[1],state[2],state[3],state[4]] , 0.5*prob ]   ]
        ...
        # return 0.5*prob*( step_cost + kill_monster_reward(state[3])) + 0.5*prob*( step_cost + kill_monster_reward(state[3]))
        return step_cost*prob
        # return [  [ [state[0],state[1],state[2],state[3],0] , 0.5*prob ] , [ [state[0],state[1],state[2],state[3],state[4]] , 0.5*prob ]   ]
        # return [  [ state[0]+state[1]+state[2]+state[3]+0 , 0.5*prob ] , [ state[0]+state[1]+state[2]+state[3]+state[4] , 0.5*prob ]   ]



def ret_val_reward(state, action, index):
    # print(state)
    # print(action)
    if(action=='NONE' or state[3]==0):
        return 0
    if(state[4]==0):
        if(state[0] == 4):
            if(action=='UP'):
